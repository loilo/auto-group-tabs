...
test('Add URL patterns via "add link" button and Enter key', async ({
  ...
  const urlPatternInputs = page.locator('.textfield')
  await expect(urlPatternInputs).toHaveCount(0)

  // Find "add link" button and click it
  const addLinkButton = page.locator('.action-new-matcher')
  await addLinkButton.click()

  // After clicking the button, there should be one input
  const urlPatternInput = page.locator('.textfield')
  ...
  const urlPatternInputsAfterAddLink = page.locator('.textfield')
  ...
  expect(await lasturlPatternInputAfterAddLink.evaluate(isFocused)).toBe(true)

  // Expect the new empty input to disappear again when focusing anything else
  await page.click('body')

  await expect(page.locator('.textfield')).toHaveCount(1)
